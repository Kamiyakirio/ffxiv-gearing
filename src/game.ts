  ...
  if (!(cacheKey in capsCache)) {
    ...
    for (const stat of Object.keys(statNames) as Stat[]) {
      ...
      caps[stat] =
        stat === 'DLY'
          ? Infinity
          : Math.round((levelCaps[stat][levelCapsIndex[level]] * slotCaps[stat][slot] * roleCaps[stat][role]) / 100000);
    }
...
const slotSchemaCombat = [{ slot: 13, name: '武器', levelWeight: 2, uiGroup: 'weapon' }, ...slotSchemaCommon];
...
export const jobSchemas = {
  ...
  BLU: {
    name: '青魔法师',
    stats: statSchemas.dpsInt,
    slots: [{ slot: 13, name: '武器', levelWeight: 0, uiGroup: 'weapon' }, ...slotSchemaCommon],
...
export const races = ['人族', '精灵族', '拉拉菲尔族', '猫魅族', '鲁加族', '敖龙族', '硌狮族', '维埃拉族'];
...
export const syncLevelIsPopular: { [index: number]: boolean } = {
  ...
  635: true,
  735: true,
  795: true, // Ultimate Raids
  300: true,
  700: true, // Field Operations
